tpl 1.19 module cmdb_ext_ComputerSystem_Case_Lower;

metadata
    origin := "community";
    description := 'Update attributes to lower case';
    last_update := '08 June 2018';
    tree_path := 'community', 'CMDBsync', 'Extended', 'ComputerSystem', 'LowerCase';
end metadata;
...
syncmapping ext_cmdb_ComputerSystem_lowercase 1.0
    """ Modify and/or add attributes once the TKU has run.
        ...
        1        Convert name, serialnumber, shortdescription, domain and host name to lower case
    ...
    body
        ...
        if device.dns_domain then
            dns_domain := text.lower(device.dns_domain);
        else
            dns_domain := "";
        end if;
        
        if device.serial then
            serial := text.lower(device.serial);
        else
            serial := "";
        end if;
        
        if device.name <> "" then
            normalized_name := text.lower(device.name);
        else
            normalized_name := device.name;
        end if;

        ci_HostName := text.lower(normalized_name); 
